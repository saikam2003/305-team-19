...
fileID = fopen('custom_pipe_data.mif', 'w');
...
for row = 0 : height-1
    for col = 0 : width-1
       address = bitshift(row, 6) + col; % Combine row and column into a 17-bit address (9 bits for row, 8 bits for col)
       address_bin = dec2bin(address, 15); % Convert address to 17-bit binary string
